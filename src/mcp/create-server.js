...
export function createArcturianLatticeMcpServer({
  ...
  tool(
    ...
    {
      cnpj: z.string().min(1),
      role: z.enum(["supplier", "organization"]).default("supplier"),
      dateFrom: z.string(),
      dateTo: z.string(),
  ...
  tool(
    ...
    async ({ cnpj }) => {
      ...
      if (!d.valid) {
        return fail("INVALID_CNPJ", "CNPJ check digits are invalid.", d);
      }
  ...
  tool(
    ...
    async ({ cnpj, legalName, dateFrom, dateTo, tribunal, page, pageSize }) => {
      ...
      const p = new JudiciarioMcpProvider({
        fetchFn,
        endpoint: env.JUDICIARIO_MCP_URL ?? null,
        bearerToken: env.JUDICIARIO_BEARER_TOKEN ?? null,
      });
      ...
      return {
        ...
        meta: {
          ...
          sourceType: env.JUDICIARIO_MCP_URL
    ...
    async ({ cnpj, legalName, limit }) => {
      ...
      return {
        ...
        meta: {
          ...
          sourceType: "official_dataset_mirror",
          estimated: false,
        },
      };
    },
  ...
  tool(
    ...
    async (args) => {
      const p = new JudiciarioMcpProvider({
        ...
        endpoint: env.JUDICIARIO_MCP_URL ?? null,
        bearerToken: env.JUDICIARIO_BEARER_TOKEN ?? null,
        publicDjenUrl: env.DJEN_PUBLIC_URL ?? undefined,
      });
      return {
        ...(await p.searchByOab(args)),
        meta: {
          source: "DJEN-CNJ",
          sourceType: "public_judicial_data",
          estimated: false,
          legalConclusion: false,
        },
      };
  ...
  tool(
    ...
    async ({ cnpj }) => {
      ...
      const [company, partners, currentProfile] = await Promise.all([
        q.company(d.normalized),
        q.partners(d.normalized),

        profileService.getByCnpj(d.normalized).catch(() => null),
      ]);

      const companyOk = company?.ok === true;

      const partnersOk = partners?.ok === true;

      const upstreamLegalName = company?.data?.cnpj_info?.razao_social ?? null;

      const currentLegalName = currentProfile?.identity?.legalName ?? null;
      ...
      if (upstreamLegalName && currentLegalName) {
        identityMatch =
          normalizeName(upstreamLegalName) === normalizeName(currentLegalName);
      }

      const sourceOk = companyOk || partnersOk;

      const status = !sourceOk
        ? "failed"
        : identityMatch === false
          ? "partial_success"
      ...
      return {
        ...
        identityValidation: {
          ...
          current: {
            ...
            tradeName: currentProfile?.identity?.tradeName ?? null,
            source: currentProfile?.meta?.source ?? null,
          },

          queridoDiario: {
            legalName: upstreamLegalName,
            tradeName: company?.data?.cnpj_info?.nome_fantasia ?? null,
          },
        ...
        meta: {
          ...
          estimated: false,
          identityCrossChecked: true,
          currentIdentitySource: currentProfile?.meta?.source ?? null,
        },
      };
    },
  );
  ...
  tool(
    ...
    async ({ domain }) => ({
      ...
      meta: {
        sourceType: "observed_and_derived",
        estimated: false,
      },
    }),
  );
  ...
  tool(
    ...
    async ({ domain, limit }) => ({
      ...
      meta: {
        ...
        sourceType: "accumulated_dataset",
        estimated: false,
      },
    }),
  );
    ...
    async ({ query, limit }) => {
      const [openalex, crossref] = await Promise.all([
        new OpenAlexProvider({
          ...
          query,
          limit,
          mailto: env.RESEARCH_CONTACT_EMAIL ?? null,
        }),
      ...
      return {
        ...
        crossref,
        meta: {
          sourceType: "open_research_metadata",
          estimated: false,
        },
      };
    },
  ...
  tool(
    ...
    async ({ query, limit }) => {
      const [web, oa, cr, news] = await Promise.all([
        searx().search({
          query,
          limit,
        }),
      ...
      return {
        ...
        news,
        meta: {
          sourceType: "multi_source_research",
          estimated: false,
        },
      };
    },
  ...
  tool(
    ...
    {
      query: z.string().min(2),
      timespan: z.string().default("1week"),
      limit: z.number().int().min(1).max(250).default(25),
    },
    async (a) => {
      const result = await new NewsSearchService({
        fetchFn,
      }).search(a);
      ...
      return {
        ...
        meta: {
          source:
            result.selectedProvider === "gdelt_doc"
              ? "GDELT DOC"
              : result.selectedProvider === "bing_news_rss"
                ? "Bing News RSS"
                : "GDELT DOC + Bing News RSS",
          sourceType: "public_news_index",
          estimated: false,
  ...
  tool(
    ...
    "Query an IBGE SIDRA table using structured table/territory/period parameters.",
    {
      ...
      level: z.union([z.string(), z.number()]).default(6),
      territories: z.string().default("all"),
      periods: z.string().default("last"),
      variables: z.string().default("all"),
    },
    async (a) => ({
  ...
  tool(
    ...
    async ({ url }) => youtube().metadata(url),
  );
  ...
  tool(
    ...
    async ({ url }) => youtube().transcript(url),
  );
  ...
  tool(
    ...
    async (a) => ({
      ...
      meta: official("Banco Central do Brasil"),
    }),
  );
  ...
  tool(
    ...
    {
      currency: z.string().min(2).default("BTC"),
    },
    async ({ currency }) => ({
      ...
      meta: {
        source: "Coinbase public endpoint",
  ...
  tool(
    ...
    {
      domains: z.array(z.string().min(1)).min(2).max(5),
    },
    async ({ domains }) =>
      new CompetitiveService({
        fetchFn,
  ...
  tool(
    ...
    {
      name: z.string().min(1),
      type: z.enum(["web_profile", "competitive_snapshot"]),
      target: z.string().min(1),
      intervalMinutes: z.number().int().min(30).max(43200).default(1440),
    },
    async (a) =>
      new MonitorService({
        db: env.DB ?? null,
  ...
  tool(
    ...
    async ({ domain, limit }) => ({
      ...
      meta: {
        ...
        sourceType: "public_certificate_data",
        estimated: false,
      },
    }),
  );
  ...
  tool(
    ...
    async ({ resource }) => ({
      ...
      meta: {
        source: "RIPEstat",
        sourceType: "public_network_data",
        estimated: false,
      },
    }),
  );
  ...
  tool(
    ...
    async ({ asn }) => ({
      ...
      meta: {
        source: "PeeringDB",
        sourceType: "public_network_registry",
        estimated: false,
      },
    }),
  );
  ...
  tool(
    ...
    async (a) =>
      new NominatimProvider({
        fetchFn,
        baseUrl: env.NOMINATIM_URL ?? null,
      }).search(a),
  );
  ...
  tool(
    ...
    {
      source: z.enum(["comexstat", "rais_caged"]),
      query: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
    },
    async (a) => ({
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        return jsonToolResult(value, isError);
      } catch (error) {
