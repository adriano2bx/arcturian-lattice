...
import { JudiciarioMcpProvider, extractMcpToolPayload, normalizeJudiciarioPublications } from "../src/providers/judiciario-mcp.js";
    ...
    return new Response(JSON.stringify({
      ...
      result: {
        structuredContent: {
          fonte: "DJEN",
          resultado: {
            items: [{ numeroProcessoFormatado: "0000000-00.2026.8.26.0000", textoLimpo: "Teste" }],
          },
        },
        isError: false,
      },
...
test("Judiciario MCP provider reports tool-level error", async () => {
  const fetchFn = async () => new Response(JSON.stringify({
    jsonrpc: "2.0",
    id: "x",
    result: { content: [{ type: "text", text: "DJEN/CNJ respondeu HTTP 504" }], isError: true },
  }), { status: 200, headers: { "content-type": "application/json" } });
  const provider = new JudiciarioMcpProvider({ fetchFn, endpoint: "https://example.test/mcp" });
...
test("normalizes DJEN publication field variants", () => {
  const out = normalizeJudiciarioPublications({ items: [{ numero_processo: "123", texto: "abc", sigla_tribunal: "TJSP" }] });
